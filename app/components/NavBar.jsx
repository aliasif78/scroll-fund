...
const NavBar = () => {
  ...
  return (
    <nav className="sticky top-0 left-0 z-[1000] p-[1rem] px-[3rem] w-full h-fit bg-[#1a1a1a] text-white flex flex-row justify-between items-center">
      ...
      {session ? (
        <button onClick={() => signOut()} className="text-white border-cyan-400 border-2 focus:ring-4 focus:outline-none focus:ring-white dark:focus:ring-white px-4 py-1.5 rounded-full transition duration-200 text-sm">
          Log Out
        </button>
      ) : (
